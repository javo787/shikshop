...
export default function LoginPage() {
  ...
  // Общая функция сохранения сессии
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      await saveAuthCookies(userCredential.user);
      router.push('/profile');
      router.refresh();
    } catch (err) {
      console.error(err);
  ...
  // 3. Успешный вход через Телефон или Telegram
       ...
       <div className="bg-white dark:bg-white/5 backdrop-blur-md p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/20">
        <h1 className="text-3xl font-serif font-bold text-center mb-2 text-dark-teal dark:text-white">Вход</h1>
         ...
         <form onSubmit={handleLogin} className="flex flex-col gap-4">
             ...
             />
             <button type="submit" disabled={loading} className="btn mt-2 w-full text-lg shadow-lg py-3 rounded-xl bg-dark-teal text-white hover:bg-opacity-90 transition-all">
         ...
         <div className="flex items-center my-6">
            ...
            <span className="px-4 text-sm text-gray-400 uppercase tracking-wide">или через</span>
            <div className="flex-1 h-px bg-gray-200 dark:bg-white/10"></div>
         </div>
         ...
         <div className="space-y-3">
            ...
            <button 
                ...
                className="w-full flex items-center justify-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 hover:bg-gray-50 dark:hover:bg-white/10 transition-colors text-gray-700 dark:text-gray-200 font-medium h-[52px]"
            >
                <GoogleIcon />
                <span>Google</span>
            </button>

            {/* Сетка: Телефон + Telegram */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
               {/* Кнопка Телефона */}
               <button 
                  type="button" onClick={() => setShowPhoneLogin(true)}
                  className="flex items-center justify-center gap-2 p-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 hover:bg-gray-50 dark:hover:bg-white/10 transition-colors text-gray-700 dark:text-gray-200 font-medium h-[52px]"
               >
                  <PhoneIcon />
                  <span>Телефон</span>
               </button>

               {/* Контейнер Telegram (специально для выравнивания) */}
               <div className="flex items-center justify-center h-[52px] border border-gray-200 dark:border-white/10 rounded-xl bg-white dark:bg-white/5 overflow-hidden">
