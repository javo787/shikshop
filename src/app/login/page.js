...
export default function LoginPage() {
  ...
  const [showPhoneLogin, setShowPhoneLogin] = useState(false); // 👇 Для модалки SMS
  
  const router = useRouter();

  // Общая функция сохранения куки
  ...
  // 3. Обработчик успешного входа через SMS/Telegram
       ...
       <div className="bg-white dark:bg-white/5 backdrop-blur-md p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/20">
        <h1 className="text-3xl font-serif font-bold text-center mb-2 text-dark-teal dark:text-white">Вход</h1>
         ...
         <form onSubmit={handleLogin} className="flex flex-col gap-4">
             ...
             />
             <button type="submit" disabled={loading} className="btn mt-2 w-full text-lg shadow-lg">
